...
// they are not expected to be present. It is a no-op when no gate
// ConfigMap exists (fresh install or same-major upgrade).
...
func AutoAcknowledgeUpgradeGatesInNamespace(
	...
	if !gateConfigMapExists(ctx, cli, operatorNS) {
		return nil
	}
...
// gateConfigMapExists returns true when the operator-created upgrade
// gate ConfigMap exists in the operator namespace.
func gateConfigMapExists(ctx context.Context, cli client.Client, namespace string) bool {
	cm := &corev1.ConfigMap{}
