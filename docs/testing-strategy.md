...
Note: `module_compliance_test.go` requires fetched Helm and Kustomize artifacts and fails with guidance to run `make get-manifests` when they are unavailable.
...
- **`module_compliance_test.go`** (`internal/controller/modules/`): Validates that every handler registered in `cmd/main.go` is included in the compliance suite; Helm and Kustomize operator artifacts render supported, uniquely identified resources without bundled platform-managed module CRs across ODH and RHOAI paths; each module renders exactly one correctly named operator Deployment with the declared container and image-injection targets; rendered CRDs are valid, structural, uniquely owned, and expose the required module status contract; and each release-specific generated module CR validates without unknown fields that Kubernetes would prune. Artifact rendering and schema checks require manifests to be fetched (`make get-manifests`).

- **`handler_compliance_test.go`** (`internal/controller/modules/`, NEW): Behavioral compliance test that exercises the registration-guarded `moduleHandlers()` set against common structural assertions: non-empty unique names, valid GVKs, manifest source presence, cluster-scoped CR/GVK consistency, nil-safety for `IsEnabled`, `BuildModuleCR`, and `WriteDSCComponentStatus`, and optional interface conformance (`ReadyConditionTyper`, `ContainerNamer`, `DeploymentNamer`, `SubmoduleConditionProvider`).
...
| Contract interfaces are validated across all implementations | Level 3: `module_compliance_test.go`, `handler_compliance_test.go` (NEW), compile-time assertions |
